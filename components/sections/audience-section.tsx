...
const integrations = [
  { name: "TDM Organisations", category: "Destination" },
  { name: "DMOs",              category: "Destination" },
  { name: "Municipalities",    category: "Public sector" },
  { name: "NTOs",              category: "National" },
  { name: "Tourism SMEs",      category: "Business" },
  { name: "Accommodation",     category: "Provider" },
  { name: "Restaurants & F&B", category: "Provider" },
  { name: "Attractions",       category: "Experience" },
];
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 lg:gap-4 mb-10">
          {integrations.map((integration, index) => (
            <div
              ...
              className={`group relative overflow-hidden flex flex-col min-h-[130px] p-4 lg:p-5 rounded-xl border transition-all duration-500 cursor-default ${
                hoveredIndex === index
                  ? "border-white/30 bg-white/[0.06] scale-[1.02]"
            ...
            >
              ...
              {hoveredIndex === index && mousePos && (
                <span
                  ...
                  style={{
                    background: `radial-gradient(220px circle at ${mousePos.x}px ${mousePos.y}px, rgba(170,215,230,0.25) 0%, transparent 70%)`,
                  }}
                />
              )}

              {/* Top row: index number (left) + category pill (right) */}
              <div className="relative z-10 flex items-start justify-between mb-3">
                <span className="font-mono text-[10px] text-[#AAD7E6]/70">
                  {String(index + 1).padStart(2, "0")}
                </span>
                <span className={`text-[9px] font-mono px-2 py-0.5 rounded-full transition-colors whitespace-nowrap ${
                  hoveredIndex === index
              ...
              {/* Spacer pushes name to bottom for consistent alignment */}
              <div className="flex-1" />

              {/* Audience name — anchored to the bottom of the card */}
              <span className="relative z-10 font-display text-base lg:text-lg text-white leading-tight">
                {integration.name}
              </span>
        ...
        <div className={`flex flex-wrap items-center justify-between gap-6 pt-6 border-t border-white/10 transition-all duration-1000 delay-500 pb-12 lg:pb-16 ${
          ...
          <div className="flex flex-wrap gap-8">
            {[
              ...
              { value: "8",   label: "audience segments" },
